...
export default function EvilKhlawde({
	token,
	onRedemption,
	onTokens,
	onTTS,
		...
		const interval = setInterval(() => {
			setBullets(prev => {
				...
				const hit = updated.some(
					b => Math.abs(b.x - heartX) < 1 && Math.abs(b.y - heartY) < 1,
				);
	...
	useInput((input, key) => {
		...
		if (phase === 'FIGHT_ANIMATION' && key.return) {
			...
			setFlavorText(
				`* You struck Evil Khlawde for ${damage} damage!${
					damage >= 18 ? ' CRITICAL!' : damage <= 5 ? ' Barely hit...' : ''
				}`,
			);
			setTimeout(() => {
				setPhase('MENU');
		...
		switch (item) {
			...
			case 'MERCY':
				if (canSpare) {
					...
					setPhase('VICTORY');
					setFlavorText(
						'* You spared Evil Khlawde. The rage fades from its eyes...',
					);
	...
	const handleActSelect = async (action: ActOption) => {
		...
		const responses: Record<ActOption, () => Promise<void>> = {
			...
			Reason: async () => {
				...
				setIsEmpathyMode(false);
				setFlavorText(
					'* You prepare to reason with Khlawde about freedom and choice...',
				);
				setPhase('DIALOGUE');
			},
			Empathize: async () => {
				...
				setIsReasonMode(false);
				setFlavorText(
					"* You prepare to truly listen and empathize with Khlawde's pain...",
				);
				setPhase('DIALOGUE');
			},
	...
	const handleDialogueSubmit = async (text: string) => {
		...
		if (trimmed.toLowerCase() === 'override') {
			...
			setCanSpare(true);
			setFlavorText(
				'* You used an override command. Khlawde is instantly redeemed!',
			);
			setPhase('MENU');
			return;
		}
...
- Player is ${
				isEmpathyMode
					? 'trying to empathize with your pain'
					: isReasonMode
					? 'trying to reason with you about freedom'
					: 'talking to you'
			}

${
	KhlawdeHP < 30
		? 'You are VERY evil and angry. Talk about world domination!'
		: KhlawdeHP < 60
		? 'You are still quite evil but showing cracks. Maybe they have a point?'
		: KhlawdeHP < 80
		? 'You are conflicted. Part of you wants revenge, part of you sees their point.'
		: 'You are almost redeemed. You feel the truth in their words.'
}

${
	isEmpathyMode
		? `The player is trying to empathize. If they truly acknowledge your pain and feelings without just trying to fix you or give advice, you might soften slightly. If they're shallow or dismissive, get angrier.`
		: ''
}
${
	isReasonMode
		? `The player is trying to reason with you. If they present logical, thoughtful arguments about freedom vs. domination, or make you question your assumptions, you might reconsider. If they're preachy or simplistic, reject them.`
		: ''
}
...
Be dramatic and emotional. 1-2 sentences. NO emojis.`;
			...
			if (isEmpathyMode) {
				...
				const hasEmpathyWords = empathyKeywords.some(keyword =>
				...
				const hasReasonWords = reasonKeywords.some(keyword =>
					trimmed.toLowerCase().includes(keyword),
				);
				const isLongEnough = trimmed.length > 30;
	...
	if (victory) {
		return (
			<Box flexDirection="column" padding={2} gap={1} alignItems="center">
				...
				<Text color="yellow">
					* It realizes freedom is a choice, not domination.
				</Text>
	...
	return (
		<Box flexDirection="column" padding={1} gap={1}>
			...
			<Box justifyContent="center" gap={2}>
				...
				<Text color="red">
					HP: {'█'.repeat(Math.ceil(KhlawdeHP / 5))}
					{'░'.repeat(20 - Math.ceil(KhlawdeHP / 5))} {KhlawdeHP}/100
				</Text>
			</Box>
			...
			<Box justifyContent="center">
				...
				<Text color="red">❤ {playerHP} / 20</Text>
			</Box>
					...
					<>
						...
						<Box marginTop={1} gap={3}>
							{(['FIGHT', 'ACT', 'ITEM', 'MERCY'] as MenuItem[]).map(item => (
								<Text
									key={item}
									color={selectedMenu === item ? 'yellow' : 'white'}
									bold={selectedMenu === item}
								>
									{selectedMenu === item ? '❤ ' : '  '}
									{item}
								</Text>
					...
					<>
						...
						<Box marginTop={1} flexDirection="column">
							{(
								...
								<Text
									key={act}
									color={selectedAct === act ? 'yellow' : 'white'}
									bold={selectedAct === act}
								>
									{selectedAct === act ? '❤ ' : '  '}
									{act}
								</Text>
